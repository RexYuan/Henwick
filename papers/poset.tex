...
Recall in section \ref{subsubsec:linear}, when encoding linear posets, we had to enforce total comparability by adding more clauses. We can avoid this since this requirement, combined with antisymmetry, is equivalent to
\[
\bigwedge_{x \neq y \in \uni} \satvar{x,y}{\poset{P}} \oplus \satvar{y,x}{\poset{P}}
\]
In fact, we can even avoid encoding linearizations altogether. To encode $\poset{P} \lext \poset{L}$, we had to encode axioms for both posets. However, since the problem input includes complete order relation $\leq[\poset{L}]$ of all given $\poset{L} \!\in\! \Upsilon$, we can skip encoding $\poset{L}$ by taking the contraposition of $\lext$ and encode $\poset{P} \lext \poset{L}$ with
\[
...
To test the limits of our method, we experimented on randomly generated input using the z3 theorem prover from Microsoft Research. Also, as input with sparse swap graph can be quickly divided and reduced to smaller problems, we restricted our input to those with single connected swap graphs.
