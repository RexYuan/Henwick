...
Since the poset cover problem is proved to be NP-complete \cite{Lenwood&Heath}, we reduce the problem to SAT to utilize the power of modern SAT solvers. However, a naive encoding would easily result in superpolynomial transformation. We shall first show where the naive encoding falls short and then present a more efficient method.
...
\]
To encode $\poset{P} \lext \poset{L}$, we had to encode axioms for both posets. However, since the problem input includes complete order relation $\leq[\poset{L}]$ of all given $\poset{L} \!\in\! \Upsilon$, we can skip the axioms and set each variables directly.
...
\comment{they've done wrong. (i think?)\\so lets solve this? i have an idea}

\subsection{Going with stages}

\comment{is this a useful idea?}
...
\begin{minted}[linenos,frame=single,fontsize=\footnotesize]{c}
#include <unistd.h>
#include <ncurses.h>
...
% NOTE: do we introduce the NP,#P complete of generation and counting lins which is what the most research has been done?
\bibliography{poset}
