...
pE = ((V['a<b'] >> (V['Labc_a<b'] & V['Lacb_a<b'] & V['Lcab_a<b'])) &
      (V['a<c'] >> (V['Labc_a<c'] & V['Lacb_a<c'] & V['Lcab_a<c'])) &
      (V['b<a'] >> (V['Labc_b<a'] & V['Lacb_b<a'] & V['Lcab_b<a'])) &
      (V['b<c'] >> (V['Labc_b<c'] & V['Lacb_b<c'] & V['Lcab_b<c'])) &
      (V['c<a'] >> (V['Labc_c<a'] & V['Lacb_c<a'] & V['Lcab_c<a'])) &
      (V['c<b'] >> (V['Labc_c<b'] & V['Lacb_c<b'] & V['Lcab_c<b'])))
exp = (pATS & pT & pE &
...
result = sat.solve(exp)
i = 1
print('---finding blanket order for [abc, acb, cab] ; expected max = { a<b }---')
while result.success:
...
pAbac = V['a<b'] | V['c<a'] | V['c<b']
pAbca = V['c<b'] | V['a<c'] | V['a<b']
#pAcab = (-V['c{a'] & -(-V['c<a'] & -V['a<c'])) | (-V['a{b'] & -(-V['a<b'] & -V['b<a']))
pAcba = V['b<c'] | V['a<b'] | V['a<c']
...
result = sat.solve(exp)
i = 1
print('---finding generating order for [abc, acb, cab] expected (a,b)---')
while result.success:
...
# NOTE: L in L(P) => cover or incomp
#       not(cover or incomp) => not(L in L(P))
#       Given we know not(L in L(P)), not(cover or incomp) can be T or F
#       so it gets us nothing.
#       If we fix not(cover or incomp), we indeed get not(L in L(P)), but it's
#       too strong. It constrains more than we ask for, because it could be the
#       case that not(L in L(P)) and (cover or incomp).
#
#       => we need find a p such that not(L in L(P)) => p
#       so that when we are given not(L in L(P)), we encode p
#
#       => sufficient condition is too much; we need the necessary condition
# TODO: find that thingy
